...
**対象:** 冒頭の前書き（`ステータス`行の直後の引用ブロック）

**(前)**

> この文書は、先に作成された`docs/specs/llm-backends.ja.md`と、その後このチャットで行われた
> Claude CodeおよびCodexのレビューを、2026-08-16に事後的にADR形式へ再構成したものである。
> （中略）元の文書は履歴資料として変更せず残す。

**(後)**

> この文書は、先に作成された草案と、その後このチャットで行われた
> Claude CodeおよびCodexのレビューを、2026-08-16に事後的にADR形式へ再構成したものである。
> （中略）元の文書の内容は本文書の`## 草案`として取り込んである。

**理由:** [ドキュメントの実装追従と整理](20260827-documentation-consistency.ja.md)のD1・D3に基づき、
`docs/specs/llm-backends.ja.md`と`docs/specs/llm-backends.md`を削除した。削除後は
「元の文書は履歴資料として変更せず残す」が事実と異なり、存在しないファイル名を指す参照も残る。
確定した仕様書に既知の誤りを残さないため訂正する。

**根拠:** 削除した`llm-backends.ja.md`は、見出しの階層（`##`→`###`）と最初の見出し名
（`状態`→`位置づけ`）を除き、本文書の`## 草案`と完全に一致していた。差分はその1行と末尾の
空行だけであり、取り込みによって失われた内容は無い。英語版`llm-backends.md`は同一内容の
翻訳であった。

**手続きについて:** 規約は改訂の対象を`最終案`と定めるが、訂正した1文は`最終案`の前に置かれた
前書きにある。文言上は対象外だが、「確定した仕様書は誤りと分かっている決定を残さない」という
趣旨からは対象であるため、同じ手続きで扱った。この判断は上記仕様の最終案・確定事項4のD2で
確定している。
