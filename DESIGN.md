...
- `feedian enrich-images`は`--limit N`か`--all`を必須とする独立工程である。本文取得と同じDNS pinning・redirect検証を使い、Browserを使わない。全resource共通の`image_ocr.workers`（既定8）で取得し、Vault設定のpath token・URL prefix、MIME・形式・アニメーション・寸法を厳しめにgateする。rasterはPillow 12.3.0でEXIF方向を反映し、長辺1,024pxを基本に文字の短辺を保つ規則でLANCZOS縮小してから、対応backendで説明画像判定と原文OCRを1 requestで行う。PNG / JPEG / GIF / WebPは必須で、WebP decoderは取得前に確認する。SVGは安全なXML解析で`text` / `tspan`だけを抽出する。`image_ocr.timeout_seconds`（既定15秒）は画像HTTP取得だけに適用し、画像LLM解析は60秒を使う。DNS解決失敗は一過性失敗として次回1回だけ再試行する。詳細と判断根拠は[画像OCRの効率化](docs/specs/20260826-image-ocr-efficiency.ja.md)を参照する。
- 取得前gate一致、取得不能、解析失敗は別の状態遷移で扱う。gate一致は完了payloadを保持して利用対象から外し、規則削除時にローカル復帰する。同じgate targetは再選択しない。404など新しいbytesを確認できない終端事象は採用中OCRをcurrentのまま維持し、新SHA-256の相違を確認できた場合だけ旧payloadを保持した`pending`へ戻す。したがってlink rotやfilter調整で保存済みOCRを破壊しない。
